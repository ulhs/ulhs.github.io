        ...
        for (const entry of body.entry) {
          ...
          for (const messagingEvent of entry.messaging) {
            ...
            if (lrn.length !== 12) {
              ...
              await sendResponse(psid, `❌ Registration Failed: The LRN ${lrn} must be exactly 12 digits.`)
            } else {
            ...
            if (text.startsWith('REG_')) {
              ...
              if (targetLrn.length === 12) {
                ...
                if (error) {
                  ...
                  await sendResponse(psid, `❌ Error unlinking student. Please try again later.`);
                } else if (data && data.length > 0) {
              ...
              if (error) {
                ...
                await sendResponse(psid, `❌ System Error: Could not link LRN ${lrn} at this time.`);
              } else if (data && data.length > 0) {
                console.log(`✅ LRN link success: ${data[0].full_name} (LRN: ${lrn})`);
                await sendConfirmation(psid, data[0].full_name, lrn);
              } else {
                console.warn(`⚠️ LRN not found (LRN Only): ${lrn}`);
                await sendResponse(psid, `❌ Registration Failed: The LRN ${lrn} was not found in our records.`);
              }
            } else {
              console.log(`❓ Unknown command from PSID ${psid}: "${rawText}"`);
              await sendResponse(psid, `🤖 Ha? Usba daw pag-type. Try sending:\n• LIST - To see linked students\n• LINK [LRN] - To add a student\n• PING - To test connection`);
            }
          }
        }
